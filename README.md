...
## Финансовый контроль и параметры риска в пилоте ASSIST

В архитектуре пилота параметры закупки и риск-лимиты рассчитываются динамически:

* **Капитальный регистр (`capital_store.py`)**: раздельный учёт для симуляции (`dry_run=True`) и реальной торговли (`dry_run=False`).
* **Начальный капитал**: команда `/seed СУММА ИСТОЧНИК` фиксирует начальный взнос (до 1 000 ₽ на старте пилота).
* **Подтверждение баланса**: команда `/cash СУММА ИСТОЧНИК` подтверждает доступные деньги по независимой выписке. Без свежего подтверждения реальные закупки блокируются (`BALANCE_UNKNOWN`).
* **Риск-гейт (`resale_intelligence/models/risk_gate.py`, `trade_admission.py`)**:
  * `reserve_ratio = 40%` (минимальный резерв капитала);
  * `lot_cap = 25%` (лимит на один лот);
  * `supplier_cap = 25%`, `category_cap = 40%`;
  * `loss_stop_ratio = 10%` (стоп-лосс от максимального капитала сессии);
  * **Emergency Stop**: немедленная блокировка новых закупок (fail-closed в риск-гейте и action-слое).
* **Критерии выхода (`exit_policy.py`)**: минимальный чистый профит ≥ 10 ₽, ROI ≥ 15%.
* **Legacy-переменные** (`FLIPPER_MAX_BUDGET`, `FLIPPER_MIN_PROFIT`, `FLIPPER_MIN_MARGIN_PCT`): являются устаревшими фоллбеками и не подменяют динамическую риск-политику.
