...
def check_capital(db, dry_run, review, category_id, price, *, conn=None, available_cash=None):
    state = db.capital_snapshot(dry_run, supplier_group=review['supplier_group'], category_id=category_id, conn=conn)
    ...
    status = db.capital_status(dry_run, conn=conn)
